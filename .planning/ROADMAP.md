...
annotation test with a passcode-gated reviewer screen. Phase 1 corrects all seven
answer-key inconsistencies before any scoring code exists. Phase 2 builds and verifies
the pure scoring engine in isolation. Phases 3-5 layer on UI, persistence, and integrity
features. Phase 6 locks in quality across the full stack before the test goes live.
...
- [ ] **Phase 2: Scoring Engine** - Build and verify the pure `scoreRound()` function and `useScoring` hook in isolation, before any UI exists
...
- [ ] **Phase 4: Results, Persistence & Design** - Results screen, backend submission, localStorage safeguard, leaderboard, badges, and full flagmail1 design language
- [ ] **Phase 5: Reviewer & Integrity** - Passcode-gated reviewer screen and session-accumulating proctoring violation tracking
...
**Goal**: A deterministic, verified scoring function exists that correctly scores any annotation for any of the 10 scenarios, including all edge cases, with no UI dependency
**Depends on**: Phase 1
...
### Phase 4: Results, Persistence & Design
**Goal**: A candidate receives a complete results screen with score, band, per-scenario breakdown, badges, and leaderboard; the full submission is persisted to localStorage and the Google Sheets backend; the entire app uses flagmail1's visual design language with annotation-correct copy
**Depends on**: Phase 3
**Requirements**: RESULT-01, RESULT-02, RESULT-03, LEAD-01, BADGE-01, BADGE-02, DESIGN-01, DESIGN-02
**Success Criteria** (what must be TRUE):
  ...
  2. Before every backend POST, the full annotation payload is written to `localStorage` keyed by candidate email; the POST fires with no-cors and does not block the results screen from rendering
  3. The leaderboard displays other candidates' scores fetched from the Google Sheets backend
...
### Phase 5: Reviewer & Integrity
**Goal**: A reviewer can access a passcode-gated screen showing candidate results with proctoring violation counts; proctoring violation events are tracked throughout the test and included in the score submission
**Depends on**: Phase 4
**Requirements**: REV-01, REV-02, PROCTOR-01, PROCTOR-02
**Success Criteria** (what must be TRUE):
  1. The reviewer screen is unreachable without the correct passcode; entering an incorrect passcode shows an error and does not reveal any candidate data
  2. The reviewer screen displays a table of candidates with their score, band, and total proctoring violation count
  3. Tab-switch and focus-loss events during the test increment a violation counter that persists across all 10 scenarios and is included in the GAS submission payload
**Plans**: TBD
**UI hint**: yes
...
| 5. Reviewer & Integrity | 0/TBD | Not started | - |
